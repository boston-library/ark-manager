...
RSpec.describe IdOrPidConstraint, type: :constraint do
  subject { described_class }

  it { is_expected.to be_const_defined(:ID_REGEX) }
  it { is_expected.to be_const_defined(:PID_REGEX) }

  describe 'instance' do
    subject { described_class.new }

    let(:expected_pattern) { Regexp.union(described_class.const_get(:ID_REGEX), described_class.const_get(:PID_REGEX)) }

    it_behaves_like 'route_constraint'
