...
namespace :boston_library do
  ...
  desc "binstub runs bundler - Your 'bin/bundle' was not generated by Bundler"
  task :binstubs_bundler do
    on roles(:app) do
      execute("cd /home/#{fetch(:user)}/#{fetch(:application)}/current; #{fetch(:rvm_installed)} #{fetch(:rvm_ruby_version)} do bundle binstubs bundler --force")
    end
  end
